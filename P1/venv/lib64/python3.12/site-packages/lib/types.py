...
class TestResult(BaseModel):
    ...
    model_config = ConfigDict(
        json_encoders={TestStatus: lambda v: v.value},
    )
    ...
    def to_dict(self) -> dict:
        key = f"Q{self.Q}" if self.Q >= 0 else "Failed to run tests"
        out = {
            key: {
                "Q": self.Q,
        ...
        if self.error:
            out[key]["error"] = self.error.to_dict()
        return out
...
class AutobadgerResult(BaseModel):
    ...
    model_config = ConfigDict(
        json_encoders={datetime: lambda v: v.isoformat(), Project: lambda v: v.value},
    )
    ...
    def model_validate(
        ...
        *,
        ...
        out = {**obj}
        tests = list({**obj["tests"]}.values())  # dict of tests
        tests = [
            {
                **test,
                "error": {
                    **test["error"],
                    "stacktrace": "\n".join(test["error"]["stacktrace"])
                    if test["error"].get("stacktrace") is not None
                    else None,
                }
        ...
        return super().model_validate(
            out, strict=strict, from_attributes=from_attributes, context=context
        )
    ...
    @classmethod
    def Zero(
        cls, project: Project, stack_trace: str, total: int, Q: int = -1
    ) -> "AutobadgerResult":
        return cls(
            project=project,
            score=Score(earned=0, total=total),
